...
Population :: Population(int CRO, int GEN, int rank) :   
   rnd(),
   n_cro(CRO),
   n_gen(GEN)
    {
    ...
    if(rank != 0){
   	ifstream Primes("Primes");
 	if (Primes.is_open()){
        for(int i=0; i<4+rank; i++) //cambio riga ad ogni rank
  	    Primes >> p1 >> p2 ;
  	 }else cerr << "PROBLEM: Unable to open Primes" << endl;
   	Primes.close();
...
void Population :: Scambia(int *nuovo){
    for(int igen = 0; igen < n_gen; igen++) popolazione[0][igen] = nuovo[igen];
};  
