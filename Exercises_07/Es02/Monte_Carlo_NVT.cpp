...
int main(int argc,char **argv){ 
  ...
  if(start == 0)
   Input();             //Inizialization
    ...
    Restart(); //Riparto da vecchia configurazione
...
// contribution to energy and virial
 ...
 epot.open("Epot.dat", ios::app);
 pres.open("Pres.dat", ios::app);
  
  epot <<  walker[iv]/(double)npart + vtail << endl; //Istant Potential energy
  pres <<  walker[iv] + vtail << endl; //Istant Potential energy
...
//g(r)
    for(int i = igofr; i < n_props; i++){
        ...
        if(iblk == nblk)
        Gave << setw(wd)  << (i-2)*bin_size << setw(wd) << stima_gdir << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gdir << endl;
    }
