...
//    double pos = ((v-m)*(v-m))/(2*s*s);
//    double neg = ((v+m)*(v+m))/(2*s*s);
//	return exp(-1*pos)+exp(-1*neg);
}
...
//    double der = (-1/(s*s))* (exp(-1*pos)+exp(-1*neg))+ (1/(s*s*s*s))* ((v-m)*(v-m)*exp(-1*pos) + (v+m)*(v+m)*exp(-1*neg));
////    	return der;
}
